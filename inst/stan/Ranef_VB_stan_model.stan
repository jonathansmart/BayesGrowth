...
model {
 ...
 sigma_L_inf ~ uniform(0, priors[5]);
  ...
  // Likelihood
  for (i in 1:N) {
    target += normal_lpdf(length[i] | L_inf_j[individual[i]] - (L_inf_j[individual[i]] - L0_j[individual[i]])*exp(-K_j[individual[i]]*age[i]), sigma);
  ...
  generated quantities {
    vector[N] log_lik;
    for (i in 1:N) {
      log_lik[i] = normal_lpdf(length[i] | L_inf_j[individual[i]] - (L_inf_j[individual[i]] - L0_j[individual[i]])*exp(-K_j[individual[i]]*age[i]), sigma);
    }
